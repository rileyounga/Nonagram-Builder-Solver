...
def nonogram_solver(rows, cols):
    ...
    while True:
        ...
        i = 0
        for col in cols:
            if 2 in [row[i] for row in grid]:
                col_grid = [row[i] for row in grid]
                grid_col = permute_union(col, height, col_grid)
...
def permute_union(vector, length, grid_vector):
    ...
    """
    vsum = sum(vector)
    vector_rep = "-".join(str(x) for x in vector)
    if vector_rep in memo:
        valid_perm = memo[vector_rep]
        valid_perm = valid_2(valid_perm, grid_vector)
    else:
        raw_perm = raw_permute(length, length - vsum, vsum)
        valid_perm = valid(raw_perm, vector)
        memo[vector_rep] = valid_perm
        valid_perm = valid_2(valid_perm, grid_vector)
...
def valid(perm, vector):
    ...
    if len(permutations) == 0:
        print("v1", perm, vector)
    return permutations


def valid_2(perm, grid_vector):
    """
    Filter out all permutations that do not match the vector
    and the grid_vector
    :param perm:
...
def main():
    ...
    rows_15 = [[8], [2, 2], [1, 1], [3, 3], [1, 11], [1, 11], [13], [11], [9], [3], [1, 5], [8, 1], [1, 3, 3], [7, 1],
               [11]]
    cols_15 = [[0], [5], [1, 1, 2, 1, 1], [1, 6, 3, 1], [1, 5, 1, 2], [1, 5, 2, 2], [1, 11], [1, 11], [1, 11],
               [1, 5, 2, 2], [1, 5, 2], [2, 6, 1], [1, 5, 1, 1], [5, 3], [1]]
    # sample 20 x 20 nonogram
    rows_20 = [[2], [2], [1], [1, 3, 1], [1, 2, 2, 1], [2, 5, 2, 1], [2, 2, 2, 3], [3, 7, 3], [3, 2, 3, 1], [3, 10, 1],
               [2, 3, 9, 1], [2, 2, 7, 2, 1], [2, 2, 7, 2, 2], [4, 7, 4], [2, 5, 1, 1], [3, 3, 2], [3, 3, 2], [5],
               [2, 2], [7]]
    cols_20 = [[2], [7], [7], [2, 1], [6], [2, 7], [2, 2, 2, 1], [2, 2, 1, 3, 1, 2], [2, 8, 1, 1], [2, 1, 1, 11, 1],
               ...
               [3, 2], [7]]
    # sample 30 x 30 nonogram
    rows_30 = [[1, 1, 3, 4, 4, 1, 5, 3], [5, 6, 2, 4, 3], [2, 1, 3, 11, 3], [1, 2, 5, 3, 3, 4, 2], [4, 5, 5, 7, 3],
               [1, 1, 4, 1, 2, 4], [5, 2, 2, 2, 1, 4], [1, 4, 2, 1, 2, 1, 4], [4, 3, 4, 4, 3], [2, 5, 2, 2],
               [6, 1, 5, 1], [2, 4, 5, 4], [8, 2, 3, 2, 3], [2, 2, 2, 2, 1, 2, 1, 1], [9, 3, 3, 4], [11, 5, 3],
               [1, 3, 3, 2, 3, 3, 2], [8, 4, 8], [6, 3, 6], [2, 1, 4], [5, 2], [6], [4, 4, 5], [1, 1, 5, 7], [3, 15],
               [2, 1, 15], [3, 15, 1], [5, 15, 2], [2, 2, 13, 1], [1, 3, 11, 2]]
    cols_30 = [[5, 5, 6, 1, 2, 1, 3], [2, 3, 8, 12], [2, 2, 3, 3, 5, 3, 1, 2, 1], [10, 18], [2, 2, 10, 2, 3],
               [1, 2, 6, 2, 2, 2], [1, 4, 2, 8], [5, 10], [5, 3], [1, 3, 1, 4], [2, 2, 7], [6, 1, 2, 8],
               [5, 2, 2, 2, 8], [4, 4, 2, 2, 8], [3, 1, 1, 4, 2, 2, 7], [3, 1, 2, 3, 3, 6], [1, 1, 4, 2, 7],
               [3, 3, 3, 8], [2, 1, 2, 2, 2, 1, 8], [1, 1, 1, 4, 2, 2, 8], [2, 1, 1, 2, 2, 3, 8], [5, 2, 1, 3, 8],
               [2, 3, 3, 6], [7, 2, 4], [1, 3, 2], [3, 2], [1, 3, 1, 4], [2, 5, 5, 1], [1, 6, 2, 4, 3],
               [1, 6, 5, 2, 2, 1]]
